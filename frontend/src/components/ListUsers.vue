<template>
    <div class="container ">
        <div class="row" style="padding-top: 15px">
            <div class="col-sm-6">
                <h4><p class="float-start">Список пользователей</p></h4>
            </div>
        ...
        <table class="example table table-striped table-hover table-sm table-responsive">
            ...
            <tbody>
            <tr v-for="(user, i) in USERS" v-bind:key="user.id" v-bind:i="i">
                <td>{{i+1}}</td>
                ...
                <td>{{user.email}}</td>
                <td >{{user.role}}</td>
                <td>             {{ moment(user.date_joined).format("DD-MM-YYYY, HH:mm a") }}</td>
                <td>
                    <!--<a @click.prevent="ShowPopUpInfo(payment.id)" class="link-style you-mat-element">-->
                    <router-link class="link-style" :to="{name:'userdetail', params:{id:user.id}}">
                    ...
                    <router-link class="link-style" :to="{name:'useredit', params:{id:user.id}}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    ...
                    <a href="" @click.prevent="deletepayment() + DELETE_USER_FROM_API(user)">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
import moment from 'moment'
    export default {
        ...
        methods: {
            ...mapActions('users', ['GET_USERS_FROM_API', 'DELETE_USER_FROM_API']),
            showPrevious() {
