<template>
    <div class="container ">
        <div class="row" style="padding-top: 15px">
            <div class="col-sm-6">
                <h4><p class="float-start">Список расходов</p></h4>
            </div>
        ...
        <table class="example table table-striped table-hover table-sm table-responsive">
            ...
            <tbody>
            <tr v-for="(expense, i) in EXPENSES" v-bind:key="expense.id" v-bind:i="i">
                <td>{{i+1}}</td>
                <td> {{expense.customer.last_name}} {{expense.customer.first_name}}</td>
                <td>{{expense.customer.phone_number}}</td>
                <td>{{expense.usd}} $</td>
                <td>{{expense.rub}} &#x20bd;</td>
                <td>{{expense.service.name}}</td>
                <td>{{expense.plan.name}}</td>

                <td>{{ moment(expense.created_at).format("DD-MM-YYYY, HH:mm a") }}</td>
                <td>
                    <!--                    <a @click.prevent="ShowPopUpInfo(payment.id)" class="link-style you-mat-element">-->
                    <router-link class="link-style" :to="{name:'expensedetail', params:{id:expense.id}}">
                    ...
                    <router-link class="link-style" :to="{name:'expenseedit', params:{id:expense.id}}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    ...
                    <a href="" @click.prevent="deleteexpense() + DELETE_EXPENSE_FROM_API(expense)">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
